...
(function (global, factory) {
    ...
    function scroll(options) {
      return observe(
        (target, handler) => ({
          disconnect: uikitUtil.on(toScrollTargets(target), "scroll", handler, {
            passive: true,
            capture: true
          })
        }),
    ...
    function transformFn(prop, el, stops) {
      ...
      if (prop === "x" || prop === "y") {
        ...
        unit = "";
        transformFn2 = (stop) => getUnit([stop]) ? uikitUtil.toPx(stop, "width", el, true) / el.offsetWidth : uikitUtil.toFloat(stop);
      }
      if (stops.length === 1) {
