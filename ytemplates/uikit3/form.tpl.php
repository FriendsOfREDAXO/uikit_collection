...
<div id="<?= $this->objparams['form_wrap_id'] ?>" class="<?= $this->objparams['form_wrap_class'] ?>">
    ...
    if ('' != $this->objparams['form_action']) {
        ...
        // Ersetze Bootstrap-Klassen durch UIkit3-Klassen
        $form_class = $this->objparams['form_class'];
        $form_class = str_replace('form-horizontal', 'uk-form-horizontal', $form_class);
        $form_class = str_replace('form-inline', 'uk-form-stacked', $form_class); 

        echo '<form action="' . $action_url . '" method="' . $this->objparams['form_method'] . '" id="' . $this->objparams['form_name'] . '" class="' . $form_class . ' uk-form" enctype="multipart/form-data">';
    }
    ?>
